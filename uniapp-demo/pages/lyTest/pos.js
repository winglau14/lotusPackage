...
//选择条码识读字符字体
var GS_f_n = new Uint8Array([29, 102, 100]);
//设置条形码高度
//1<<n<<255默认值:50
...
async function lanyaposgo3(text, cback) {
	const deviceId = await lotusUtils.getStorageFn('deviceId');
	 const serviceId = await lotusUtils.getStorageFn('serviceId');
	 const characteristicId = await lotusUtils.getStorageFn('characteristicId');
    uni.writeBLECharacteristicValue({
        deviceId: deviceId||LY.dayinji.uuid,
        serviceId: serviceId||LY.dayinji.suuid,
        characteristicId: characteristicId||LY.dayinji.nuuid,
        value: text.buffer,
        success: function (res) {
        },
        fail: function (res) {
...
function ZhengChu(_len1, _len2) {
    return new Math.ceil(_len1 / _len2)
}

async function lanyaposgo(text, cback) {
	const deviceId = await lotusUtils.getStorageFn('deviceId');
	 const serviceId = await lotusUtils.getStorageFn('serviceId');
	 const characteristicId = await lotusUtils.getStorageFn('characteristicId');
    ...
    uni.writeBLECharacteristicValue({
        ...
        fail: function (res) {
            isPrint = false;
            cback("err");
            console.log("打印错误:"+JSON.stringify(res))
            console.log("打印数据:"+"/" + text + "/")
        }
    })
}
        ...
        case 'Print'.toLowerCase()://打印
            if (value.length > 8) {
                ...
                for (var x = 0; x < cl; x++) {
                    ...
                    //console.log("本s次:", cd)
                    if (x == (cl - 1)) {
                        QueueWrite.push(Arry2Arry(Hex2Arry(encodeToGb2312(cd)), Print()))
                    }
                    else {
                ...
                QueueWrite.push(Arry2Arry(Hex2Arry(encodeToGb2312(value)), Print()))
            }
            break;
