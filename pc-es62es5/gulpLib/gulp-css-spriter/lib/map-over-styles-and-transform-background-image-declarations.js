...
function mapOverStylesAndTransformAllBackgroundImageDeclarations(styles, cb) {
	...
	function transformDeclaration(declaration, declarationIndex, declarations) {
        ...
        if(transformedDeclaration.property === 'background-image' && /\?__sp/i.test(transformedDeclaration.value)) {

            transformedDeclaration.value = transformedDeclaration.value.replace('?__sp','');
            return cb(transformedDeclaration, declarationIndex, declarations);
        }
        // Background is a shorthand property so make sure `url()` is in there 且判断url是否有?__spriter后缀
        else if(transformedDeclaration.property === 'background' && /\?__sp/i.test(transformedDeclaration.value)) {
