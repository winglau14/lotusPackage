 ...
 **/
export const lotusPagination = {
    init: function (options) {
        let parentDom = document.getElementById(options.parentDom);
        let showPageTotalNum = document.getElementById(options.leftPageTotal);
        let dom = document.getElementById(options.domItemList);
        let prev = document.getElementById(options.prevBtn);
        let next = document.getElementById(options.nextBtn);
        let enSureBtn = document.getElementById(options.enSureBtn);
        let showTotalNum = document.getElementById(options.rightPageTotal);
        let curNum = document.getElementById(options.curNum);
        //计算翻页的页数
        let totalPage = Math.ceil(options.totalNum / options.showPageSize);
        //是否隐藏左侧共有多少记录模块
        ...
        //console.log(lotusMultiPagination);
        //前台页面共有多少条数据显示
        ...
        function createDom(page) {
            //console.log(page, lotusMultiPagination.index);
            //console.log(`显示...需要减去item的个数：${page - 7}`);
            //`显示...需要减去item的个数：${page - 7}`
            let removeItem = page - options.maxShowNum;
            //页码字符串拼接
            let str = '';
            let tempItemArr = [];
            ...
            //给item绑定click事件
            let tarDom = dom.getElementsByTagName('a');
            for (let i = 0; i < tarDom.length; i++) {
                tarDom[i].onclick = function () {
                    //console.log(this.innerText);
                    options.index = parseInt(this.innerText) - 1;
                    curNum.value = options.index + 1;
                    //console.log(`剩余：${totalPage - (lotusMultiPagination.index + 1)}`);
                    //页码数据重组
                    pageCommonFn();
                    //回调函数
                    if (options) {
